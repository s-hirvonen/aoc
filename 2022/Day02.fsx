...
let part1 = input |> Seq.sumBy ((fun row -> (toChoice row.[0], toChoice row.[1])) >> score) 
let part2 = input |> Seq.sumBy ((fun row -> (toChoice row.[0], choiceFromResult (toChoice row.[0]) (toResult row.[1]))) >> score)
