...
let rec solve count (cave: int[,]) =
    ...
    match cave[500 - offset, 0] with
    | 2 -> count
    | _ -> solve (count + 1) (drop (500 - offset) 0 cave)
...
|> fun (cave, walls) -> walls |> Seq.collect (Seq.pairwise >> (Seq.collect allPoints) >> Seq.distinct) |> Seq.fold (setPoint (0 - offset) 1) cave
|> addFloor |> solve 0
