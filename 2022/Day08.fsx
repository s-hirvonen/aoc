let input = System.IO.File.ReadAllLines "2022/inputs/day08.txt" |> Seq.map (Seq.map (System.Char.ToString >> int)) |> array2D

let isVisible row i value = [Seq.take i row; Seq.skip (i + 1) row] |> Seq.map (Seq.forall ((>)value))
...
let solve operation combinator = 
    Array2D.mapi (fun y x i -> [(operation input.[*,x] y i); (operation input.[y,*] x i)] |> Seq.concat |> Seq.reduce combinator)
